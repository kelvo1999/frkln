...
$proimg=md5($img).$extension;
    ...
    // $query=mysqli_query($con, "insert into artist(Name,MobileNumber,Email,Education,Award,Profilepic) value('$name','$mobnum','$email','$edudetails','$awarddetails','$proimg')");
    $query = $con->prepare("INSERT INTO artist (Name, MobileNumber, Email, Education, Award, Profilepic) VALUES (?, ?, ?, ?, ?, ?)");
    $query->bind_param("ssssss", $name, $mobnum, $email, $edudetails, $awarddetails, $proimg); //preparing statements prevents sql injection 😂😂
    $query->execute();
    
    if ($query) {
echo "<script>alert('Artist details has been added.');</script>";
echo "<script>window.location.href ='manage-blog-artist.php'</script>";
  }
  else
    {
